...
class MQTTSubscriber:
    ...
    def start(self) -> None:
        ...
        try:
            ...
            logger.info(f"Podłączono do brokera na {self.host}:{self.port}")
            ...
            self._sock.close()
            logger.info("Zakończono działanie subskrybenta.")
    ...
    def _send_subscribe(self) -> bool:
        """Wysyła SUBSCRIBE i czeka na SUBACK. Zwraca True jeśli sukces."""
        logger.info("Wysyłam pakiet SUBSCRIBE...")
        self._sock.sendall(
        ...
        try:
            ...
            logger.error("Brak odpowiedzi na SUBSCRIBE (Timeout).")
        return False
        ...
        while True:
            try:
                ...
                if msg_type != self.MSG_PUBLISH:
                    logger.warning(f"Nieoczekiwany typ pakietu: {msg_type}. Ignoruję.")
                    continue
                ...
                logger.info(f"[SEND] PUBACK  | Pakiet {packet_id} potwierdzony.")
...
if __name__ == "__main__":
    sensor_filter = int(sys.argv[1]) if len(sys.argv) > 1 else 0
    sub = MQTTSubscriber(sensor_filter=sensor_filter)
    sub.start()
